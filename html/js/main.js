...
(function () {
	...
	function renderWorks() {
		...
		WORKS.forEach(function (w, i) {
			...
			html += '<' + tag + ' class="work-card' + (hasLink ? '' : ' no-link') + '" data-cat="' + w.cat + '"' + attrs + '>' +
				'<span class="work-thumb ph-' + (i % 5) + '">' +
					'<img src="' + IMG + w.img + '" alt="' + w.name + '" loading="lazy" data-name="' + w.name + '">' +
				'</span>' +
	...
	function initFilter() {
		...
		filter.addEventListener('click', function (e) {
			...
			if (!btn) return;
			filter.querySelectorAll('.filter-btn').forEach(function (b) { b.classList.remove('is-active'); });
